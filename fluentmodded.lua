-- fluent but auto save setting toggle
...
ElementsTable.Toggle = (function()
	...
	function Element:New(Idx, Config)
		...
		Creator.AddSignal(ToggleFrame.Frame.MouseButton1Click, function()
			local ppp = not Toggle.Value
			Toggle:SetValue(ppp)
			_G.noguchi_env[2][Config.Title] = ppp
			print(Config.Title, ppp)
		end)
		if _G.noguchi_env["ohshit"] then
			_G.noguchi_env[2][Config.Title] = Toggle.Value
		end
		Toggle:SetValue(Toggle.Value)

		Library.Options[Idx] = Toggle
		_G.noguchi_env[3][Config.Title] = Toggle
		return Toggle
	end
	return Element
end)()
...
_G.noguchi_env = {[1] = {}, [2]={}, [3]={}}
_G.noguchi_env["GetPath"] = _G.noguchi_env["GetPath"] or nil
_G.noguchi_env["GetFolderPath"] = _G.noguchi_env["GetFolderPath"] or nil
_G.noguchi_env["GetFilePath"] = _G.noguchi_env["GetFilePath"] or nil
_G.noguchi_env["ohshit"] = false
_G.noguchi_env[2] = setmetatable({}, {
	__newindex = function(self, i, v)
		self = _G.noguchi_env
		pcall(function()
			if not self[1]["FolderName"] or not self[1]["FileName"] then print("thua r") return end
			print("????") 
			local thua = string.format("%s/%s", self[1]["FolderName"], self[1]["FileName"])
			if isfolder(self[1]["FolderName"]) and isfile(string.format("%s/%s", self[1]["FolderName"], self[1]["FileName"])) then 
				local ok, res = pcall(function()
					return hs:JSONDecode(readfile(thua))
				end)
				if ok then 
					res[i] = v
					writefile(thua, hs:JSONEncode(res))
					if _G.noguchi_env["ohshit"] then
						local oldconfig = string.format("%s/%s", self[1]["FolderName"], "OLD_" .. self[1]["FileName"])
						if not isfile(oldconfig) then
							writefile(oldconfig, '{"null": false}')
						end
						writefile(oldconfig, hs:JSONEncode(res))
					end
				else					
					writefile(thua, '{"null": false}')
				end
			else
				_G.noguchi_env["ohshit"] = true
				writefile(thua, '{"null": false}')
				local ok, res = pcall(function()
					return hs:JSONDecode(readfile(thua))
				end)
...
setmetatable(_G.noguchi_env, {
    __call = function(self, v1, v2)
        print("Fluent modded by noguchi hyuga")
        _G.noguchi_env[1]["FolderName"] = v1
        _G.noguchi_env[1]["FileName"] = v2
        local thua = string.format("%s/%s", _G.noguchi_env[1]["FolderName"], _G.noguchi_env[1]["FileName"])
        if not _G.noguchi_env["GetPath"] then
            _G.noguchi_env["GetPath"] = function()
                return thua
            end
        end
        if not _G.noguchi_env["GetFolderPath"] then
            _G.noguchi_env["GetFolderPath"] = function()
                return self[1]["FolderName"]
            end
        end
        if not _G.noguchi_env["GetFilePath"] then
            _G.noguchi_env["GetFilePath"] = function()
                return self[1]["FileName"]
            end
        end
    end
})
...
function Library:CreateWindow(Config)
	...
	_G.noguchi_env(Config.FolderName and type(Config.FolderName) == "string" and Config.FolderName or "Noguchi", Config.FileName and type(Config.FileName) == "string" and Config.FileName or "config.save")
...
function Library:LoadConfig()
	...
	repeat
		wait()
	until _G.noguchi_env["GetPath"]
	print("true 2")
    if _G.noguchi_env["GetPath"] then
		print("true 3")
        local oc = _G.noguchi_env["GetPath"]()
        local on, gg = pcall(function()
            return game.HttpService:JSONDecode(readfile(oc))
        end)
		-- for i,v in pairs(gg) do
		-- 	print(i, v, _G.noguchi_env[3][i])
		-- 	for i_, v_ in pairs(_G.noguchi_env[3][i]) do
		-- 		print(i_, v_)
		-- 	end
		-- end
        for i, v in pairs(gg) do
			-- print(i, v)
			-- print(_G.noguchi_env[3][i])
			if _G.noguchi_env[3][i] then
				_G.noguchi_env[3][i]:SetValue(v)
				print("changed",i,v)
			end
        end
    end
...
function Library:ResetConfig()
	if _G.noguchi_env["GetFilePath"] and _G.noguchi_env["GetFolderPath"] then
		local oc = string.format("%s/%s", _G.noguchi_env["GetFolderPath"](), _G.noguchi_env["GetFilePath"]())
		if isfile(oc) then
			local on, gg = pcall(function()
				return hs:JSONDecode(readfile(oc))
			end)
